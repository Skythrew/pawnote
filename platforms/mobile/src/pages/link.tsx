...
import { callAPIUsingCapacitor } from "@/utils/client/requests/fetcher";
...
const Page: Component = () => {
  ...
  const handleGeolocation = async (): Promise<void> => {
    try {
      ...
      const data = await callAPIUsingCapacitor<ApiGeolocation>({
  ...
  return (
    <>
      <header class="sticky top-0 z-10 mb-6 flex items-center justify-start gap-2 border-b-2 border-b-latteSubtext1 bg-latteBase px-2 py-4">
        <A href="/" class="flex items-center justify-center px-4 py-2 text-xl" >
          <IconMdiArrowLeft />
        </A>
