...
const AppHomeworks: Component = () => {
  ...
  const BaseFilterToggleButton: Component<{
    ...
    <label
      ...
      class="cursor-pointer flex w-full py-3 px-4 transition gap-2 items-center md:(py-1 !rounded-full) first:rounded-t-md last:rounded-b-md "
    >
      <input type="checkbox"
        checked={props.active}
        onChange={evt => props.action(evt.currentTarget.checked)}
      />
      <span class="text-brand-dark truncate dark:text-brand-white">{props.label}</span>
    </label>
  );
          ...
          <div class="flex my-8 w-full justify-center relative md:px-4">
            ...
            >
              ...
              <Transition appear show={showFiltersMenu() || windowWidth() >= 768}>
                ...
                >
                  <div class="bg-brand-white rounded-t-md flex flex-col w-full p-8 gap-4 md:(p-0 pr-8) dark:bg-dark-500 dark:md:bg-brand-dark "
